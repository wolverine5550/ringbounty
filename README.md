...
| Rate limiting (§2.7) | In-DB sliding windows via `public.consume_rate_limit` ([`src/lib/rate-limit/`](src/lib/rate-limit/)). [`POST /api/check/submit`](src/app/api/check/submit/route.ts) enforces **10 checks/hour per `rb_anonymous_sid`** and **30/hour per IP** (tunable in [`constants.ts`](src/lib/rate-limit/constants.ts)); returns **429** with friendly copy. **Company Intelligence enqueue (CI-1.4):** same hourly caps per session/IP on Lane B (`company_intelligence_enqueue` action); skip enqueue + server log when exceeded (does not block `/check`). CAPTCHA stub: [`src/lib/rate-limit/captcha.ts`](src/lib/rate-limit/captcha.ts). **Before marketing traffic + full agent:** Cloudflare Turnstile (**O1** in [`docs/ongoing_task_manager.md`](docs/ongoing_task_manager.md)) in addition to these caps. |
...
| **FTC complaints** | **ETL script shipped (CI-2.1)** — not run in prod until counsel sign-off (**CI-P.2.4**). Live API + bulk CSV: `Subject` = **complaint category only** ([`docs/spikes/20260519120000-ftc-bulk-csv-schema-validation.md`](docs/spikes/20260519120000-ftc-bulk-csv-schema-validation.md)). **Path B:** phone + modal category + count in `seed_violations.metadata`; `reported_company_name` null. **Manual ingest:** `npm run etl:ftc-dnc -- --file ./docs/DNC_Complaint_Numbers_YYYY-MM-DD.csv` or `--date YYYY-MM-DD` (downloads from FTC). Add `--dry-run` to parse without upsert. Loads `SUPABASE_SECRET_KEY` from `.env.local` automatically (CLI scripts do not use Next’s env loader). **Monthly refresh:** re-run with all CSVs in your rolling window (e.g. 30 weekdays → ~200k–400k phones). Agent Round 1 lookup: **CI-2.2**. |
| **Company Intelligence Agent** | **Schema + enqueue + worker + cron + rate limits + orchestrator (Rounds 1–4 SerpAPI + OpenRouter) + optional complaint-site scrape + callback recursive lookup + suggest-only persistence + cost logging + qualify Screen 4 UX + check funnel background copy** (**CI-0.1**, **CI-1.1**–**CI-1.4**, **CI-2.2**, **CI-3.1**–**CI-3.3**, **CI-4.1**–**CI-4.3**, **CI-5.1**, **CI-6.1**, **CI-6.2**, **CI-8.1**–**CI-8.5**). **CI-8.5:** Playwright E2E for Screen 4 voicemail CTA + mocked company-intel poll — [`e2e/qualify-screen-4-company-intel.spec.ts`](e2e/qualify-screen-4-company-intel.spec.ts) (`E2E_USER_EMAIL`, `E2E_USER_PASSWORD`). Round 3: [`serpapi-complaint-search.ts`](src/lib/company-intelligence/sources/serpapi-complaint-search.ts) (`SERPAPI_API_KEY`, `COMPANY_INTEL_SERP_ENABLED`). **CI-5.1:** [`scrape-complaint-sites.ts`](src/lib/company-intelligence/sources/scrape-complaint-sites.ts) (`COMPANY_INTEL_SCRAPE_ENABLED`, default off — [`legal spike`](docs/spikes/20260519210000-complaint-site-scrape-legal-review.md)). Round 4: [`synthesize-company-from-sources.ts`](src/lib/company-intelligence/synthesize-company-from-sources.ts) (`OPENROUTER_API_KEY`, optional `COMPANY_INTEL_OPENROUTER_MODEL`). **CI-6.1:** after parent synthesis, up to two callback E.164s enqueue child runs (`parent_run_id`); child agent-only lookup may set parent `company_name_suggested` at confidence 90 — [`enqueue-callback-intelligence-runs.ts`](src/lib/company-intelligence/enqueue-callback-intelligence-runs.ts), [`apply-callback-resolution-to-parent.ts`](src/lib/company-intelligence/apply-callback-resolution-to-parent.ts). **CI-6.2:** qualify voicemail / Screen 4 callback phone triggers same child pipeline — [`enqueue-qualify-callback-intelligence-run.ts`](src/lib/company-intelligence/enqueue-qualify-callback-intelligence-run.ts); results show callback attribution ([`results-subject-card.tsx`](src/components/results/results-subject-card.tsx)). **CI-8.1–8.2:** Screen 4 polls [`GET /api/qualify/company-intel`](src/app/api/qualify/company-intel/route.ts) every 3s while pending/running; shows researching banner, suggest pre-fill + confidence badge, and primary voicemail CTA when unidentified — [`screen-4-company-form.tsx`](src/components/qualify/screen-4-company-form.tsx), [`use-qualify-company-intel-poll.ts`](src/lib/qualify/use-qualify-company-intel-poll.ts). **CI-8.3:** `/check` shows non-blocking “researching in the background” when Lane B enqueues (`company_intel_enqueued` on `number_checks`) — [`check-funnel-client.tsx`](src/components/check/check-funnel-client.tsx). **CI-8.4:** `/results` subject cards show `company_intel_reasoning` when present; attorney evidence PDF includes Lane B source tiers + reasoning (no raw scrape payloads) — [`results-subject-card.tsx`](src/components/results/results-subject-card.tsx), [`format-company-intel-evidence.ts`](src/lib/company-intelligence/format-company-intel-evidence.ts). **Cost (CI-4.3):** per-run `estimated_cost_cents` + `apis_called[]` on `company_intelligence_runs` and matching `claim_events` keys — [`company-intel-run-cost.ts`](src/lib/company-intelligence/company-intel-run-cost.ts); tune defaults via `COMPANY_INTEL_SERPAPI_COST_CENTS` / `COMPANY_INTEL_OPENROUTER_COST_CENTS`. Worker suggest + `claim_events`: [`persist-company-intelligence-outcome.ts`](src/lib/company-intelligence/persist-company-intelligence-outcome.ts) (v1 never sets `company_identified`; v2 optional `COMPANY_INTEL_AUTO_PROMOTE_ENABLED`). Orchestrator: [`run-company-intelligence-agent.ts`](src/lib/company-intelligence/run-company-intelligence-agent.ts) — Round 1 `seed_violations`, Round 2 reuse `metadata.spam_providers` (no Nomorobo HTTP; stale after 24h / `COMPANY_INTEL_LANE_A_REUSE_MAX_AGE_MS`), short-circuit at `COMPANY_INTEL_SHORT_CIRCUIT_THRESHOLD` (default 70). Round 1: [`querySeedViolations`](src/lib/company-intelligence/sources/seed-violations.ts) (populate via [`etl:ftc-dnc`](package.json)). Paid rounds gated by sign-in (**CI-P.5**). Enqueue: [`maybeEnqueueCompanyIntelligenceRun`](src/lib/company-intelligence/enqueue-company-intelligence-run.ts) — gated by **10/hour per session + 30/hour per IP** ([`assert-company-intelligence-enqueue-allowed.ts`](src/lib/rate-limit/assert-company-intelligence-enqueue-allowed.ts)); fail-open skip + log when capped. Optional fail-open fetch after enqueue (**CI-1.3.3**). **Cron drain (source of truth):** [`GET /api/cron/company-intelligence-drain`](src/app/api/cron/company-intelligence-drain/route.ts) — Vercel Cron every **2 min** when `COMPANY_INTELLIGENCE_AGENT_ENABLED=true` ([`vercel.json`](vercel.json)). **Turnstile (O1)** required before marketing scale — see post-MVP backlog. Env: `CRON_SECRET`, `COMPANY_INTELLIGENCE_AGENT_ENABLED`, optional `COMPANY_INTEL_CRON_BATCH_SIZE`. Checklist: [`docs/company_id_task_manager.md`](docs/company_id_task_manager.md). |
...
| **`/check` (§6.4.4)** | Unidentified copy, “Company identified” when Nomorobo hits, **unverified CNAM/hint** line when `company_name_hint` present. **CI-8.3:** background company-research status when Lane B enqueues (does not block qualify). |
...
| **Results §8.4** | [`/results`](src/app/(post-check)/results/page.tsx): [`loadResultsPageContext`](src/lib/claims/load-results-page-context.ts) — strength, valuation, referral gates; subject cards (spam/DNC + **CI-8.4** Lane B “Company research” reasoning when set) + strength header + valuation panel + ineligible/email capture + attorney CTA ([`AttorneyReferralCta`](src/components/results/attorney-referral-cta.tsx) → `/attorney-connect` when eligible). |
...
| **Attorney §13.2** | [`runEvidencePdfJob`](src/lib/leads/run-evidence-pdf-job.ts) → [`generateAndUploadEvidencePdf`](src/lib/leads/evidence-pdf/generate-and-upload-evidence-pdf.ts) (pdfkit) → private `lead-packages` bucket; `leads.evidence_pdf_url` = `lead-packages:{leadId}/evidence-package.pdf`. Per-subject **CI-8.4** “Automated company research” section when Lane B completed (source tiers, reasoning, APIs — no raw scrape). `/results` shows [`AttorneySharingChecklist`](src/components/results/attorney-sharing-checklist.tsx). |
...
**Company Intelligence — ops cost monitoring (CI-4.3.2):**

| Signal | Suggested staging alert | Notes |
|--------|-------------------------|-------|
| Daily `sum(estimated_cost_cents)` on completed runs | **> 500 cents ($5)** | Tune after first week with `COMPANY_INTELLIGENCE_AGENT_ENABLED=true`. |
| Paid API rate | **> 50%** of completed runs have `apis_called` non-empty while `company_name_suggested` stays null | May indicate SerpAPI/OpenRouter spend without identification lift — review prompts or short-circuit threshold. |
| Query | `select sum(estimated_cost_cents), count(*) from company_intelligence_runs where status = 'completed' and created_at > now() - interval '1 day';` | Mirrors `claim_events` keys `company_intel_estimated_cost_cents` / `company_intel_apis_called` per completion. |
...
The wiring spec does not start Next.js. **CI-8.5** qualify Screen 4 specs need `npm run dev`, Supabase env, and `E2E_USER_*` (see [`e2e/README.md`](e2e/README.md)). Run wiring only: `npm run test:e2e -- --project=wiring`.
