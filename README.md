...
| **Company Intelligence Agent** | **Schema shipped** (**CI-0.1**); worker/UX not shipped. Tables: `seed_violations` (phone PK, FTC/agent cache), `company_intelligence_runs` (async queue + audit), `claim_subjects.company_name_suggested` / `company_intel_*` (suggest-only). RPC: `claim_company_intelligence_runs` (`service_role` only). Migration: [`supabase/migrations/20260519193000_company_intelligence_schema.sql`](supabase/migrations/20260519193000_company_intelligence_schema.sql). Checklist: [`docs/company_id_task_manager.md`](docs/company_id_task_manager.md). **CI-P.4:** `COMPANY_INTEL_AUTO_PROMOTE_ENABLED=false`. **CI-P.5–6:** paid-round gate + cron claim — [`paid-intel-rounds.ts`](src/lib/company-intelligence/paid-intel-rounds.ts), [`worker-policy.ts`](src/lib/company-intelligence/worker-policy.ts). Env (when **CI-1** ships): `COMPANY_INTELLIGENCE_AGENT_ENABLED`, `CRON_SECRET`. Policy: [`docs/company-identification-strategy.md`](docs/company-identification-strategy.md). |
