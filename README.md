...
| **Company Intelligence Agent** | **Planned** (not shipped). Checklist: [`docs/company_id_task_manager.md`](docs/company_id_task_manager.md) (**CI-P** → **CI-11**); product spec: [`docs/claude_chat_company_finder.md`](docs/claude_chat_company_finder.md). v1: async research during qualify, **suggest-only** for `company_name` until user confirms Q13. **CI-P.3:** provenance on `claim_events` (`company_identification_source=company_intelligence`); Lane A spam keeps `company_name_source=nomorobo` — audit: [`docs/spikes/20260519140000-company-identification-source-audit.md`](docs/spikes/20260519140000-company-identification-source-audit.md). **CI-P.4:** `COMPANY_INTEL_AUTO_PROMOTE_ENABLED=false` (default); [`shouldPromoteToIdentified`](src/lib/company-intelligence/confidence.ts) blocks auto `company_identified`. **CI-P.5:** anonymous checks may enqueue runs; paid SerpAPI/synthesis only when signed in — [`shouldRunPaidIntelRounds`](src/lib/company-intelligence/paid-intel-rounds.ts), `COMPANY_INTEL_ALLOW_ANONYMOUS_PAID_ROUNDS=false`. **CI-P.6:** cron drain required (`POST /api/cron/company-intelligence-drain`, `CRON_SECRET`); claim via `admin.rpc('claim_company_intelligence_runs')` + `FOR UPDATE SKIP LOCKED` (not filter-only select); retry policy [`worker-policy.ts`](src/lib/company-intelligence/worker-policy.ts) — design: [`docs/spikes/20260519160000-company-intel-worker-design.md`](docs/spikes/20260519160000-company-intel-worker-design.md). Turnstile (**O1**) before marketing + full agent: [`docs/ongoing_task_manager.md`](docs/ongoing_task_manager.md). Baseline metrics: [`docs/spikes/20260519150000-company-intel-step4-baseline-metrics.md`](docs/spikes/20260519150000-company-intel-step4-baseline-metrics.md). Policy: [`docs/company-identification-strategy.md`](docs/company-identification-strategy.md). |
