...
| **Company Intelligence Agent** | **Schema + enqueue + worker + cron + rate limits + Round 1 seed** (**CI-0.1**, **CI-1.1**–**CI-1.4**, **CI-2.2**); SerpAPI/synthesis rounds (**CI-4**) not shipped. Round 1: [`querySeedViolations`](src/lib/company-intelligence/sources/seed-violations.ts) on `seed_violations` (populate via [`etl:ftc-dnc`](package.json)). Paid rounds gated by sign-in (**CI-P.5**). Enqueue: [`maybeEnqueueCompanyIntelligenceRun`](src/lib/company-intelligence/enqueue-company-intelligence-run.ts) — gated by **10/hour per session + 30/hour per IP** ([`assert-company-intelligence-enqueue-allowed.ts`](src/lib/rate-limit/assert-company-intelligence-enqueue-allowed.ts)); fail-open skip + log when capped. Optional fail-open fetch after enqueue (**CI-1.3.3**). **Cron drain (source of truth):** [`GET /api/cron/company-intelligence-drain`](src/app/api/cron/company-intelligence-drain/route.ts) — Vercel Cron every **2 min** when `COMPANY_INTELLIGENCE_AGENT_ENABLED=true` ([`vercel.json`](vercel.json)). **Turnstile (O1)** required before marketing scale — see post-MVP backlog. Env: `CRON_SECRET`, `COMPANY_INTELLIGENCE_AGENT_ENABLED`, optional `COMPANY_INTEL_CRON_BATCH_SIZE`. Checklist: [`docs/company_id_task_manager.md`](docs/company_id_task_manager.md). |
