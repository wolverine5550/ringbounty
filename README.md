...
| **Company Intelligence Agent** | **Schema + enqueue + worker** (**CI-0.1**, **CI-1.1**, **CI-1.2**); agent rounds/UX not shipped. Enqueue: [`maybeEnqueueCompanyIntelligenceRun`](src/lib/company-intelligence/enqueue-company-intelligence-run.ts). Worker: `POST /api/internal/company-intelligence/run` (`Authorization: Bearer $CRON_SECRET`; body `{ "run_id" }` or `{ "batch_size": 5 }`) — [`process-company-intelligence-run.ts`](src/lib/company-intelligence/process-company-intelligence-run.ts). Flags default **off**. Cron drain: **CI-1.3**. Checklist: [`docs/company_id_task_manager.md`](docs/company_id_task_manager.md). Policy: [`docs/company-identification-strategy.md`](docs/company-identification-strategy.md). |
