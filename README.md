...
| **Attorney §13.1** | [`/attorney-connect?claim=`](src/app/(post-check)/attorney-connect/page.tsx) — expectations + consent → [`POST /api/leads/attorney-referral`](src/app/api/leads/attorney-referral/route.ts) creates `leads` (`status=new`), records eligible `claim_subjects` on `claim_events`, queues evidence PDF (§13.2 stub). Confirmation email via Resend when `RESEND_API_KEY` is set. |
| **Next** | §13.2 evidence PDF for firms. |
