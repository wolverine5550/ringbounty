...
| **Qualification (§6.2 + §7.1)** | [`/qualify/[claimSubjectId]`](src/app/(post-check)/qualify/[claimSubjectId]/page.tsx) — federal DNC pre-gate ([`FederalDncAttestationForm`](src/components/qualify/federal-dnc-attestation-form.tsx)), then wizard screens **1–4** via `?step=` ([`qualify-step.ts`](src/lib/qualify/qualify-step.ts), [`load-qualify-context.ts`](src/lib/qualify/load-qualify-context.ts)). Optional `?claim=` must match subject's claim. Resume: `claim_events` key `qualify_step_resume`. [`POST /api/qualify/federal-dnc`](src/app/api/qualify/federal-dnc/route.ts) → `dnc_check_results` / `claim_events`. |
