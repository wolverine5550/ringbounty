...
| **Results §8.4** | [`/results`](src/app/(post-check)/results/page.tsx): [`loadResultsPageContext`](src/lib/claims/load-results-page-context.ts) computes strength (weakest-link until §8.5), valuation, referral gates; subject cards + strength header + valuation panel + ineligible/email capture + attorney CTA ([`AttorneyReferralCta`](src/components/results/attorney-referral-cta.tsx) — weak ack checkbox; lead flow §13.1). |
| **Next** | §8.5 persist `claim_strength` + `estimated_*_cents`, §13.1 attorney lead flow. |
