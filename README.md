...
**v0.1 consumer path (2026-05-18):** `/check` → screen numbers → qualify → `/results` → optional `/attorney-connect`. PRD §10 **evidence preservation** (screenshots, call logs, notes) runs on **attorney connect**, not on `/check`, so users confirm the number and complete qualification before the “build your evidence profile” checklist gates attorney referral.

### Anonymous funnel (Phase §2.3–§2.4)

| Piece | Location / notes |
|-------|-------------------|
| `/check` layout (§4.1–§4.6, §5.4) | Mobile-first funnel at [`src/app/check/page.tsx`](src/app/check/page.tsx): [`CheckPageShell`](src/components/check/check-page-shell.tsx); single-step **Enter numbers** in [`CheckFunnelClient`](src/components/check/check-funnel-client.tsx) (§4.3–§4.4 masked US NANP rows, max 10, validation, duplicate hints). **No Step 0 evidence checklist on `/check`** — that moved to [`/attorney-connect`](src/app/(post-check)/attorney-connect/page.tsx) ([`EvidencePreservationChecklist`](src/components/evidence/evidence-preservation-checklist.tsx)). Helpers + server validation: [`us-phone.ts`](src/lib/check/us-phone.ts). After submit: per-number **`number_checks`**, **Next step** → qualify ([`check-funnel-continue.ts`](src/lib/check/check-funnel-continue.ts)); [`CheckOutcomePanel`](src/components/check-outcome-panel.tsx) shows gate copy + **Check another number**. Phone input ids use React `useId()` for SSR hydration. [`POST /api/check/submit`](src/app/api/check/submit/route.ts) returns `claim_id`, `claim_subject_ids`, `number_checks`; dispatches `RB_CHECK_SUBMITTED_EVENT` ([`constants.ts`](src/lib/check/constants.ts)). |
...
| Gated routes (§2.5.2) | Anonymous users with a successful query are redirected to the account wall from [`/results`](src/app/(post-check)/results/page.tsx), [`/summary`](src/app/(post-check)/summary/page.tsx), and [`/qualify/[claimSubjectId]`](src/app/(post-check)/qualify/[claimSubjectId]/page.tsx) via [`enforcePostCheckAccess`](src/lib/claims/enforce-post-check-access.ts). Legacy [`/letter/*`](src/app/(post-check)/letter/[[...slug]]/page.tsx) redirects to `/results`. Proxy allows these paths through [`isAnonymousAllowedPath`](src/lib/anonymous-session.ts). |
...
**v0.1 product direction (2026-05-17, funnel UX 2026-05-18):** No DIY demand letters or consumer Stripe Checkout. Users **screen numbers on `/check`**, **qualify** on `/qualify/...`, see **informational claim strength** on `/results`, and may **connect with an attorney** when [`canReferToAttorney`](src/lib/claims/can-refer-to-attorney.ts) passes. **Evidence preservation** (PRD §10 checklist) runs on **`/attorney-connect`** before referral so the firm-facing PDF reflects intentional prep. Phases 9–10 (letter purchase) are cancelled; see `task_manager.md`.
