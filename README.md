...
| **Company Intelligence Agent** | **Schema + enqueue + worker + cron** (**CI-0.1**, **CI-1.1**–**CI-1.3**); agent rounds/UX not shipped. Enqueue: [`maybeEnqueueCompanyIntelligenceRun`](src/lib/company-intelligence/enqueue-company-intelligence-run.ts) (optional fail-open fetch to worker after enqueue — **CI-1.3.3**). **Cron drain (source of truth):** [`GET /api/cron/company-intelligence-drain`](src/app/api/cron/company-intelligence-drain/route.ts) — Vercel Cron every **2 min** when `COMPANY_INTELLIGENCE_AGENT_ENABLED=true` ([`vercel.json`](vercel.json); Vercel sends `Authorization: Bearer $CRON_SECRET`). Internal worker: `POST /api/internal/company-intelligence/run` (body `{ "run_id" }` or `{ "batch_size": 5 }`). Env: `CRON_SECRET`, `COMPANY_INTELLIGENCE_AGENT_ENABLED`, optional `COMPANY_INTEL_CRON_BATCH_SIZE`. Checklist: [`docs/company_id_task_manager.md`](docs/company_id_task_manager.md). Policy: [`docs/company-identification-strategy.md`](docs/company-identification-strategy.md). |
