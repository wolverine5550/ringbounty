...
|-------|-------------------|
| `/check` layout (§4.1–4.2) | Mobile-first funnel at [`src/app/check/page.tsx`](src/app/check/page.tsx): [`CheckPageShell`](src/components/check/check-page-shell.tsx); step state + §4.2 evidence checklist in [`CheckFunnelClient`](src/components/check/check-funnel-client.tsx) ([`CheckStepIndicator`](src/components/check/check-step-indicator.tsx): Step 0 — Preserve evidence → Step 1 — Enter numbers). Checklist rows + supportive copy in [`src/lib/check/evidence-checklist-items.ts`](src/lib/check/evidence-checklist-items.ts); continue gating in [`evidence-checklist-gate.ts`](src/lib/check/evidence-checklist-gate.ts). Step headings in [`src/lib/check/constants.ts`](src/lib/check/constants.ts). Phone inputs ship §4.3. |
